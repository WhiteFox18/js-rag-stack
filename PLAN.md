...
- Persistence naming: physical database table names, database columns, and
  Prisma persistence fields use singular `snake_case`. Keep Prisma model names
  in PascalCase and map every model to its table with `@@map`; do not use
  field-level `@map` or convert database records to camelCase.
...
All physical table names use singular `snake_case` (`user`,
`anonymous_session`, `auth_session`, `chat`, and `message`). Prisma model names
remain PascalCase and declare explicit `@@map` mappings. All persisted field
names in the Prisma schema use `snake_case`, so Prisma inputs and returned
database records also expose `snake_case` properties.
